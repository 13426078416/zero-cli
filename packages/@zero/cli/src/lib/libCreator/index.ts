...
class LibCreator extends BaseCreator {
  ...
  public async create(): Promise<void> {
    ...
    adaptedPreset.plugins["@zero-cli/cli-lib-service"] = { projectName: this.name };
    ...
    const pkg: BasePkgFields = {
      ...
      version: "0.0.1",
      devDependencies: {},
      ["__zero_config__"]: { projectName: this.name, ...adaptedPreset },
      engines: {
        node: ">=10",
      },
    };
    ...
    deps.forEach((dep: string) => {
      ...
      const packageDirNameMatchResult = /^@zero-cli\/(cli-(plugin|lib)-.+)$/.exec(dep);
    ...
    // so that cli-service-plugin can setup git hooks.
