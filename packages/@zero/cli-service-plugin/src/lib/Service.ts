...
import { error, loadFile } from "@zero-cli/cli-utils-shared";
...
class Service {
  ...
  constructor(context: string) {
    ...
    this.PROJECT_CONFIG_FILE_NAME = "zero.config.ts";
  ...
  private async resolveConfigPlugins(): Promise<ConfigPlugin[]> {
    ...
    const prefixRE = /^@zero-cli\/cli-plugin-/;
    ...
    const pluginList = this.pkg.__zero_config__ ? this.pkg.__zero_config__.plugins : {};
    ...
    if (chainableConfig) {
      ...
      configQueue?.rawCallback.forEach((fn) => {
        ...
        if (typeof fn === "function") {
          ...
          // but in 'zero.config.ts' just allow modify ‘module’ 'plugins' 'externals', because we have type check
